...
class DefaultInvoice extends \Magento\Sales\Model\Order\Pdf\Items\AbstractItems
{
    ...
    public function draw()
    {
		...
		$internal_sku = $product->getInternalSku();
        $pdf = $this->getPdf();
        $page = $this->getPage();
        $lines = [];
        ...
        // draw Product name
        $lines[0][] = [
                //'text' => $this->string->split($this->prepareText((string)$item->getName()), 40, true, true),
				'text' => '',
                'feed' => 35
        ];
		
		$lines[1][] = [
               // 'text' => $this->string->split($this->prepareText((string)'SKU: '.$internal_sku), 50, true, true),
			    'text' => $this->prepareText((string)'SKU: '.$internal_sku),
                'feed' => 35
        ];
