...
class Order extends AbstractModel implements EntityInterface, OrderInterface
{
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/breezelogger.log');
		$logger = new \Zend\Log\Logger();
		$logger->addWriter($writer);
		$logger->info('-----start order file-----');
        $this->_eventManager->dispatch('sales_order_place_before', ['order' => $this]);
		$logger->info('-----before payment-----');
        $this->_placePayment();
		$logger->info('-----after payment-----');
        $this->_eventManager->dispatch('sales_order_place_after', ['order' => $this]);
		$logger->info('-----after last event dispatch-----');
		$logger->info('-----end order file-----');
        return $this;
    }
