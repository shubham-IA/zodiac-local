...
abstract class AbstractPdf extends \Magento\Framework\DataObject
{
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
       ...
       // $page->setFillColor(new \Zend_Pdf_Color_GrayScale(1));
		...
		$page->setFillColor(new \Zend_Pdf_Color_Rgb(0.93, 0.92, 0.92));
            $page->setLineWidth(0.5);
            $page->drawRectangle(25, $this->y, 275, $this->y - 30);
			...
			if($currencycode=='INR'){
				...
				$curcode = $order->getOrderCurrencyCode();
			}
			...
			$mrp = round($order->getGrandTotal(),2);
             $page->drawText(__('Total Order Quantity: '.round($order->getData('total_qty_ordered'),4)), 35, $this->y, 'UTF-8');
            $page->drawText(__('Total Order Value: INR'.$mrp), 285, $this->y, 'UTF-8');
        ...
        if ($order->getIsVirtual()) {
          ...
          //  $page->drawRectangle(275, $this->y, 570, $this->y - 25);
			...
			if($order->getStatus()=='pgcomplete'){
				$orderstatus = 'PG Complete';
			}else {
				$orderstatus = $order->getStatus();
			}
            $page->drawText(__('Order Status: '.ucfirst($orderstatus)), 35, $this->y, 'UTF-8');
           // $page->drawText(__('Total Order Value: INR'.$mrp), 285, $this->y, 'UTF-8');
    ...
    public function drawLineBlocks(\Zend_Pdf_Page $page, array $draw, array $pageSettings = [])
    {
        ...
        foreach ($draw as $itemsProp) {
            ...
            $height = isset($itemsProp['height']) ? $itemsProp['height'] : 8;
